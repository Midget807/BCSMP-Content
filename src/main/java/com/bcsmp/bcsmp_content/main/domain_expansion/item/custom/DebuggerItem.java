...
public class DebuggerItem extends Item {
            ...
            if (!player.isSneaking()) {
                WorldRenderEvents.AFTER_ENTITIES.register(context -> {
                    if (debugState.shiftBool) {
                        this.renderSides(context, box, 1.0f, 1.0f, 1.0f);
                    }
                });
    ...
    private void renderSides(WorldRenderContext context, ExpansionBox box, float r, float g, float b) {
            ...
            */
            if (camX > box.getBoundEast() - viewDistance) {
                ...
                for (double t = beforeNorthBound; t < beforeSouthBound; iteration += 0.5f) {
                    ...
                    builder.vertex((float) (box.getBoundEast() - camX), (float) (box.getBoundDown() - camY), (float) (t - camZ)).texture(1 - iteration, (float) (time + (fraction + 0.25 * box.getBoundUp())));
                    builder.vertex((float) (box.getBoundEast() - camX), (float) (box.getBoundDown() - camY), (float) (t + u - camZ)).texture(1 - (v + iteration), (float) (time + (fraction + 0.25 * box.getBoundUp())));
            ...
            */
            if (camX < box.getBoundWest() + viewDistance) {
                ...
                for (double t = beforeNorthBound; t < beforeSouthBound; iteration += 0.5f) {
                    double u = Math.min(1.0, beforeSouthBound - t);
                    float v = (float)u * 0.5F;
                    builder.vertex((float) (box.getBoundWest() - camX), (float) (box.getBoundDown() - camY), (float) (t - camZ)).texture(1 + iteration, (float) (time + (fraction + 0.25 * box.getBoundUp())));
                    builder.vertex((float) (box.getBoundWest() - camX), (float) (box.getBoundDown() - camY), (float) (t + u - camZ)).texture(1 + (v + iteration), (float) (time + (fraction + 0.25 * box.getBoundUp())));
            ...
            if (camZ > box.getBoundSouth() - viewDistance) {
                ...
                for (double t = beforeWestBound; t < beforeEastBound; iteration += 0.5f) {
                    ...
                    float v = (float)u * 0.5F;
                    builder.vertex((float) (t - camX), (float) (box.getBoundDown() - camY), (float) (box.getBoundSouth() - camZ)).texture(1 + iteration, (float) (time + (fraction + 0.25 * box.getBoundUp())));
                    builder.vertex((float) (t + u - camX), (float) (box.getBoundDown() - camY), (float) (box.getBoundSouth() - camZ)).texture(1 + (v + iteration), (float) (time + (fraction + 0.25 * box.getBoundUp())));
    ...
    private void renderEnds(WorldRenderContext context, ExpansionBox box, float r, float g, float b) {
        double viewDistance = context.worldRenderer().client.options.getClampedViewDistance() * 16;
        ServerTickEvents.START_SERVER_TICK.register(server -> {
            box.tick();
        });
